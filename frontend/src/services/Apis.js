...
export const usergetfunc = async(search,price,status,sort,page)=>{
    return await commonrequest("GET",`${BASE_URL}/user/details?search=${search}&price=${price}&status=${status}&sort=${sort}&page=${page}`,"");
}
...
export const importcsvfunc = async (formData) => {
    return await commonrequest("POST", `${BASE_URL}/user/import`, formData);
}
